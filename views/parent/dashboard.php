...
<div class="p-4 lg:p-10 font-sans pb-20">
    <div class="max-w-6xl mx-auto">
        ...
        <div class="bg-white rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden mb-10">
            <div class="px-10 py-8 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div class="flex items-center gap-3">
                    <div class="p-2 bg-emerald-50 text-emerald-600 rounded-xl"><i data-lucide="history" class="w-5 h-5"></i></div>
                    <h3 class="text-xl font-black text-slate-800 tracking-tight italic">Histórico de Movimentações</h3>
                </div>
                
                <form action="" method="GET" class="flex gap-2">
                    <input type="hidden" name="child_id" value="<?= $selectedId ?>">
                    <select name="month" onchange="this.form.submit()" class="bg-slate-50 border border-slate-200 text-slate-600 text-xs font-bold rounded-xl px-3 py-2 outline-none focus:border-emerald-500">
                        <?php for($m=1; $m<=12; $m++): $mm = str_pad($m, 2, '0', STR_PAD_LEFT); ?>
                            <option value="<?= $mm ?>" <?= $filterMonth == $mm ? 'selected' : '' ?>><?= $mm ?></option>
                        <?php endfor; ?>
                    </select>
                    <select name="year" onchange="this.form.submit()" class="bg-slate-50 border border-slate-200 text-slate-600 text-xs font-bold rounded-xl px-3 py-2 outline-none focus:border-emerald-500">
                ...
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-slate-50">
                        <?php if (empty($history)): ?>
                            <tr><td colspan="4" class="px-10 py-8 text-center text-slate-400 text-sm font-bold italic">Nenhuma movimentação neste período.</td></tr>
                        <?php else: ?>
...
<div id="modalEditStudent" class="modal-overlay">
    <div class="modal-content bg-white rounded-[2.5rem] relative shadow-2xl">
        <button onclick="closeModal('modalEditStudent')" class="absolute right-6 top-6 text-slate-300 hover:text-slate-500"><i data-lucide="x"></i></button>
        ...
        <form onsubmit="handleFormSubmit(event, 'update_child.php', 'modalEditStudent')">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="text-slate-400 uppercase ml-2 font-black">CPF</label>
                    <input type="text" name="cpf" value="<?= $childData['cpf'] ?>" required class="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none focus:border-emerald-500 font-bold text-slate-700">
                </div>
                <div>
                    <label class="text-slate-400 uppercase ml-2 font-black">Senha Login</label>
                    <input type="password" name="password" class="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none focus:border-emerald-500 font-bold text-slate-700" placeholder="••••••">
            ...
            <button type="submit" class="submit-btn w-full bg-[#10b981] text-white font-black rounded-2xl shadow-xl hover:scale-[1.02] transition-all italic uppercase tracking-widest text-xs mt-2">Salvar</button>
        </form>
    </div>
</div>
